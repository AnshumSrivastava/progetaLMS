	...
	<div class="flex flex-col md:flex-row gap-8">
		...
		<aside class="w-full md:w-64 shrink-0">
			...
			<nav class="flex flex-col gap-2">
				...
				{#if role === 'admin' || role === 'owner'}
					...
					>
						<LayoutDashboard size={18} /> Admin Preferences
					</button>
				{/if}
			</nav>
		</aside>
		...
		<main class="flex-1 bg-white rounded-lg shadow-sm border border-gray-100 p-8">
			...
			{#if activeTab === 'profile'}
				<div>
					<h3 class="text-xl font-bold text-gray-800 mb-2 border-b pb-4">My Profile</h3>
					<p class="text-sm text-gray-500 mb-6">Update your personal information and display name.</p>
					...
					<div class="bg-gray-50 p-5 rounded-lg border border-gray-200">
						...
						<form method="POST" action="?/updateProfile" use:enhance class="max-w-md space-y-4">
							<div>
								<label for="name" class="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
								<input type="text" id="name" name="name" value={data.user?.name || ''} required placeholder="e.g. John Doe"
